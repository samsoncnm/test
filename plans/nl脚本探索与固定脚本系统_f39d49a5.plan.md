...
**报告解析机制**（来自 llms-full.txt）：
- `agent.reportFile` 属性直接暴露本次会话生成的 HTML 报告文件路径
- `splitReportFile({ htmlPath, outputDir })` 是**同步**函数，返回 `{ executionJsonFiles: string[] }`，JSON 结构以实际结果为准
- 探索会话结束后，只需解析整份报告即可获得所有动作序列

#### 4.1.1 类型扩展
修改 `src/types/index.ts`：

```typescript
// ExplorationStep 新增 reportPath，指向本次 aiAct 对应的报告文件
export interface ExplorationStep {
  ...
  deepLocate?: boolean;
  errorMessage?: string;
  reportPath?: string;  // 本次 aiAct 生成的报告文件路径（来自 agent.reportFile）
}
```

#### 4.1.2 修改 midscene-adapter.ts：记录报告路径 + deepLocate 支持
修改 `src/core/midscene-adapter.ts` 的 `executeAndLog`：
...
> `agent.reportFile` 是 Midscene Agent 的内置属性，直接返回本次会话生成的 HTML 报告路径。无需额外实现 `getLastReportPath`。

#### 4.1.3 新增 report-parser.ts：从报告 JSON 提取动作序列
新增 `src/utils/report-parser.ts`：
...
export interface ParsedSubAction {
  type: string;       // Tap / Input / Scroll / Sleep 等
  target?: string;
  value?: string;
  status?: string;
}
...
> `splitReportFile` 是同步的，`executionJsonFiles` 直接可访问。JSON 结构以实际解析结果为准（文档已说明）。

#### 4.1.4 修改 yaml-freezer.ts：基于报告数据凝固
修改 `src/core/yaml-freezer.ts`，`freezeToYaml` 改为 `async`：
...
export async function freezeToYaml(params: {
  ...
  // 找到最后一个成功的 step 的报告文件，解析整份报告
  const lastSuccessStep = [...explorationLog.steps]
    .reverse()
    .find((s) => s.result === "success");

  const flow: Record<string, unknown>[] = [];

  if (lastSuccessStep?.reportPath && fs.existsSync(lastSuccessStep.reportPath)) {
    const executions = parseReportFile(lastSuccessStep.reportPath);
    for (const exec of executions) {
      if (exec.status !== "finished") continue;

      // 如果有 subActions，展开为独立的动作
      if (exec.subActions && exec.subActions.length > 0) {
        for (const sub of exec.subActions) {
          if (sub.status !== "finished") continue;
          const normalizedType = normalizeActionType(sub.type);
          flow.push({ [normalizedType]: sub.target ?? sub.value ?? exec.prompt });
        }
      } else {
        // Plan 类型或无 subActions 的任务，降级为 ai
        flow.push({ ai: exec.prompt });
      }
    }
  }
...
| ID | 任务 | 状态 | 验收 |
|----|------|------|------|
| p1-1 | `types/index.ts` ExplorationStep 新增 `reportPath?: string` 字段 | ⏳ | 类型检查通过 |
| p1-2 | `midscene-adapter.ts` executeAndLog 记录 `agent.reportFile` 报告路径 + deepLocate 支持 | ⏳ | 报告路径正确记录 |
| p1-3 | 新增 `src/utils/report-parser.ts`：调用 `splitReportFile`（同步）解析报告 JSON，提取 actionType 和 subActions | ⏳ | `splitReportFile` 不报错 |
| p1-4 | `yaml-freezer.ts` 改为异步，基于报告数据凝固为正确的动作类型 + `agent.deepLocate` | ⏳ | 凝固 YAML 包含正确动作类型（aiTap/aiInput/sleep 等） |
