...
export async function freezeToYaml(params) {
     ...
     */
    function normalizeLocate(locate) {
        if (!locate)
            return undefined;
     ...
     */
    function serializeYamlFlowItem(item) {
        const result = {};
                ...
                for (const action of exec.actions) {
                    ...
                    if (param.value !== undefined) {
                        if (lastYamlFlowItem) {
                            ...
                            const item = { [normalizedType]: "" };
                            if (param.value !== undefined)
                                item.value = param.value;
                            if (param.locate)
                                item.locate = normalizeLocate(param.locate) ?? "";
                            flow.push(item);
                        }
                    }
                    else {
                        // 无 value（如 aiTap）：直接使用 yamlFlow 的 locate（ActionSpace param.locate 是对象）
                        const locateValue = normalizeLocate(param.locate) ??
