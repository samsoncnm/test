...
function readModelConfig(prefix: string) {
 ...
 */
function injectModelEnvVar(
  prefix: string,
  config: { modelBaseUrl: string; modelApiKey: string; modelName: string; modelFamily: string },
): void {
...
export function getMidsceneConfig(): MultiModelConfig {
  ...
  const defaultConfig: MidsceneConfig = {
  ...
  return {
    ...
    planning: planningConfig as PlanningModelConfig | undefined,
    insight: insightConfig as InsightModelConfig | undefined,
  };
}
